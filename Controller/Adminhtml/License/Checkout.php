 ...
 * Stores -> Config -> eTechFlow -> Order Email Editor -> Payment Settings,
    ...
    public function execute(): ResultInterface
    {
        ...
        if (!$stripeKey || str_starts_with($stripeKey, '****')) {
            $this->messageManager->addErrorMessage(
                __('Stripe Secret Key is not configured. Go to Stores -> Config -> eTechFlow -> Order Email Editor -> Payment Settings.')
            );
            return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('order_email_editor/license/gate');
        }
        ...
        $cancelUrl  = $this->getUrl('order_email_editor/license/gate');

        $postData = http_build_query([
            'payment_method_types[0]'                              => 'card',
            'line_items[0][price_data][currency]'                  => $currency,
            'line_items[0][price_data][product_data][name]'        => $planInfo['name'] . ' — ETechFlow',
            'line_items[0][price_data][product_data][description]' => 'Order Email Editor module for ' . $domain,
