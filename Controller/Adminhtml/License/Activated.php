...
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        $block = $page->getLayout()->getBlock('etechflow.oee.license.activated');
        if ($block) {
            $block->setData('license_key', $licenseKey)
                  ->setData('plan', $planName)
                  ->setData('error', $error)
                  ->setData('settings_url', $this->getUrl('adminhtml/system_config/edit/section/etechflow_orderemaileditor'))
                  ->setData('management_url', $this->getUrl('order_email_editor/history/index'));
        }
